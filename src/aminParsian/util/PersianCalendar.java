...
public class PersianCalendar implements Serializable {
    ...
    public static String getPersianDate(int gregYear, int gregMonth, int gregDay) {
        ...
        double j_days_of_year = Math
                .floor(((d_j / len) - Math.floor(d_j / len)) * 365) + 2;

        // System.out.println(j_days_of_year);
        StringBuffer result = new StringBuffer();

        result.append((int) j_y );
        if((int) month(j_days_of_year) < 10)
            result.append("/0" + (int) month(j_days_of_year));
        else
            result.append("/" + (int) month(j_days_of_year));
        if((int) dayOfMonth(j_days_of_year)<10)
            result.append("/0"+(int) dayOfMonth(j_days_of_year));
        else
            result.append("/"+(int) dayOfMonth(j_days_of_year));
