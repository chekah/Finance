...
<display:table export="true" name="credebitList" id="credebitList" uid="credebit" class="table table-bordered displaytable"
    ...
    <%--<c:if test="${moarefiName.status.intValue() < 3}">--%>
    <%--<form method="post" action="hcPrtalEnterSabtMablaghTakhmini">--%>
    <%--<input type="hidden" name="moarefiNamehID" value="${moarefiName.moarefiID}"/>--%>
    <%--<sec:authorize access="hasRole('hcPrtalEnterSabtMablaghTakhmini')">--%>
    <%--<input type="submit" class="submitButton" theme="simple" value="ثبت ترخیص"/>--%>
    <%--</sec:authorize>--%>
    <%--</form>--%>
    <%--</c:if>--%>
    <%--</display:column>--%>
