				...
				$('.' +self.element.attr('id') +'.cg-resetButton').bind('click.combogrid', function(){
					...
					if(self.options.okIcon){
						...
						if(self.options.resetButton){
							...
							$('.' +self.element.attr('id') +'.cg-searchButton').append('<span class="'+ self.element.attr('id') +' notok-icon"></span>');
						}else{
		...
		//ESCAPE needs this workaround
					...
					if(self.options.okIcon){
						if(self.options.resetButton){
							...
							$('.' +self.element.attr('id') +'.cg-searchButton').append('<span class="'+self.element.attr('id') +' ok-icon"></span>');
						}else{
...
//                    contentType: "application/x-www-form-urlencoded; charset=UTF-8",
					data: {
                        ...
                        str:$('#queryinfo'+self.element.attr('id')).val(),   //a-s
						haveWhere: self.options.haveWhere,
						sn: self.options.sn
                    },
